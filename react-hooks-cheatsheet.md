...
## Custom Hooks
If there is complicated state or lifecycle logic that is used by multiple components, it can be placed into one function or "hook." This does ***NOT*** mean components share state with eachother between the custom hook, only that they reuse the logic (this makes it so you don't have to write duplicate logic for each component)
...
- Custom hooks should always have a lowercase `use` in the start of their name as in `useMyHook`
